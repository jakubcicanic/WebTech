...
<section id="video" class="home">
    ...
    <h1 style="font-size: 30px"><strong><?php echo $language[$_SESSION["lang"]][3] ?></strong></h1>
        ...
        <div id="tables">
            <?php 
                if(isset($_SESSION['admin']) && $_SESSION['admin'] == 1) {
                    ...
                    $sqlSelectTeam = "SELECT tim from Tim where ID_STUDENT = '".$_SESSION['logged'][0]."' AND predmet ='".$_SESSION['currentSubject']."'";
                    $conn = connectToDatabase($dbUrl, $dbLogin, $dbPass, $dbName);
                    ...
                    $teamSqlDraw = 'SELECT body FROM timy_body where tim = '.$_SESSION['loggedTeam']." AND predmet = '".$_SESSION['currentSubject']."'";
                    $captainSqlDraw = 'SELECT kapitan FROM timy_body where tim = '.$_SESSION['loggedTeam']." AND predmet = '".$_SESSION['currentSubject']."'";
                    $draw = $conn->query($teamSqlDraw);
                    ...
                    $subjectSql = "SELECT DISTINCT predmet FROM Tim where ID_STUDENT = '".$_SESSION['logged'][0]."'";
        ...
        $teamTableSql = "SELECT meno, email,body,Suhlas FROM `Tim` WHERE predmet = '".$_SESSION['currentSubject']."' AND tim = ".$_SESSION['loggedTeam'];
        $result = $conn->query($teamTableSql);
        $teamGivenCheckSql = "SELECT body FROM timy_body where predmet = '".$_SESSION['currentSubject']."' AND tim = ".$_SESSION['loggedTeam'];
        $givenCheck= $conn->query($teamGivenCheckSql);
        $givenCheckTeam = mysqli_fetch_array($givenCheck);
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if(( !is_null($givenCheckTeam['body']) && is_null($row['body']) && is_null($teamCaptain['kapitan'])) or ( !is_null($givenCheckTeam['body']) && is_null($row['body']) && $teamCaptain['kapitan'] == $_SESSION['logged'][0])) {
            echo "<td>
                <form method='post' action='rating.php'>
                <input type='number' name='studentPoints' min=\"0\" step=\"1\" oninput=\"validity.valid||(value='');\">
