...
?>
  ...
  <form method="post" action="">
   <?php echo '<h3 style="font-size: 30px"><b>'.$language[$_SESSION["lang"]][57].'</b></h3>';?>
    <select id="subject" name="Subject" onchange="getState(this.value);">
      <option value="">Select Subject</option>
      <?php
      $sql = "SELECT DISTINCT predmet FROM Tim";
      $result = $conn->query($sql);
      while($rs = $result->fetch_assoc()){

        ?>
        <option value="<?php echo $rs['predmet'];?>"> <?php echo $rs['predmet'];?> </option>
        <?php
    ...
    <br>
    <select id="teamList" name="Team" >

      <option value=""> Select Team </option>

    </select>
    <br>
    <button type="submit" class="button pulse">Submit</button>
